...
meshConfig:
  ...
  trustDomain: {{ if $.Values.global }}{{ $.Values.global.cluster }}{{ else }}{{ $.Values.cluster }}{{ end }}
components:
  pilot:
    k8s:
      env:
...
{{/*    # install istio to the release namespace by default*/}}
    ...
    network: {{ if $.Values.global }}{{ $.Values.global.cluster }}{{ else }}{{ $.Values.cluster }}{{ end }}
    # needed for annotating istio metrics with cluster (should match trust domain)
    multiCluster:
      clusterName: {{ if $.Values.global }}{{ $.Values.global.cluster }}{{ else }}{{ $.Values.cluster }}{{ end }}
